 ...
 * @ClassName: PrimaryAudit
 */
public class PrimaryAudit extends AuditChain {
    public PrimaryAudit(String id, String value, String userType) {
        super(id, value, userType);
    }

    @Override
    public ChainUser doAudit(String uid) {
        ChainUser chainUser = Chain.CHAIN_USER.get("2");
        ChainUser auditStatus = new AuditServiceImpl().getAuditStatus(chainUser.getUid());
        ChainUser user = AuditServiceImpl.req.get("001");
        if (null == auditStatus) {
            System.out.println("初审：审批人" + chainUser.getName() + "-待审批,申请人:" + user.getName());
            return null;
        }
        if ("2".equals(auditStatus.getUserType())) {
            if (uid != chainUser.getUid()) {
                return super.next.doAudit(uid);
            }
            System.out.println("初审：审批人" + chainUser.getName() + "-审批通过");
        }
        return super.next.doAudit(uid);
    }
}
