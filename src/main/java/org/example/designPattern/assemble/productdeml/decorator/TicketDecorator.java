...
import org.example.designPattern.assemble.BaseDecorator;
import org.example.designPattern.assemble.Decorate;
...
/**
 * @Author jfz
 * @Date 2024/2/26 12:19
 * @PackageName:org.example.designPattern.assemble.productdeml.decorator
 * @ClassName: TicketDecorator
 */
public class TicketDecorator extends BaseDecoratorByPrice {
    public TicketDecorator(Amount amount) {
        super(amount);
    }
    ...
    public BigDecimal getAmount(Order order) {
        ...
        Promotion red = order.getPromotionType().stream().filter(or -> or.getPromotion().equals("TICKE")).collect(Collectors.toList()).get(0);
